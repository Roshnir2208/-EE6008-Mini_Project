"""
Train a higher-capacity single-object detector for OpenMV RT1062.
V2: larger input/backbone than previous detector to improve camera-side accuracy.
...
class RT1062DetectorTrainerV2:
    def __init__(self, tfrecord_dir, output_dir):
        self.tfrecord_dir = Path(tfrecord_dir)
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(parents=True, exist_ok=True)

        # Larger than V1 to improve representation quality.
        self.img_size = 160
        self.batch_size = 8
        self.epochs = 24
        self.learning_rate = 2e-4
        self.backbone_alpha = 0.5
    ...
    def _parse_record(self, serialized):
        ...
        def select_obj_idx():
            # Pick the largest object per image for stable single-box training.
    ...
    def _augment(self, image, targets):
        ...
        image_out = tf.image.random_contrast(image_out, lower=0.85, upper=1.15)
        image_out = tf.image.random_saturation(image_out, lower=0.9, upper=1.1)
    ...
    def train(self):
        print("=" * 70)
        print("Training RT1062 detector V2")
        print("=" * 70)
        ...
        callbacks = [
            tf.keras.callbacks.ModelCheckpoint(
                str(self.output_dir / "best_detector_v2.h5"),
        ...
        model.save(str(self.output_dir / "detector_v2_final.h5"))
        print("Saved Keras model:", self.output_dir / "detector_v2_final.h5")

        return model, history

    def export_int8_tflite(self, model):
        print("\nExporting full INT8 TFLite (V2)...")
        rep_ds = tf.data.TFRecordDataset(str(self.tfrecord_dir / "train.tfrecord"))
        rep_ds = rep_ds.map(self._parse_record, num_parallel_calls=tf.data.AUTOTUNE).take(200)
        ...
        tflite_path = self.output_dir / "detector_v2_int8.tflite"
        ...
        metadata = {
            ...
            "backbone": f"MobileNetV2 alpha={self.backbone_alpha}",
        ...
        with open(self.output_dir / "detector_v2_metadata.json", "w", encoding="utf-8") as f:
...
def main():
    ...
    trainer = RT1062DetectorTrainerV2(tfrecord_dir, output_dir)
    model, _ = trainer.train()
    trainer.export_int8_tflite(model)

    print("\nDone: detector V2 training + export complete")
